...
String[][] data = read_excel_data.exceldataread("Device");
DeviceStatus device_status = new DeviceStatus();
	int Cap = device_status.Device_Status();
	
	if (data[1][1].equals("Android")) {
		Cap = Cap + 1;
	} else {
		Cap = Cap;
	}
...
public static void startAppiumServer() throws Exception {
	String[][] data = read_excel_data.exceldataread("Appium");
	CommandLine command = new CommandLine(data[1][1]);
	command.addArgument(data[2][1], false);
	command.addArgument("--address", false);
	command.addArgument(data[3][1]);
	command.addArgument("--port", false);
	command.addArgument(data[4][1]);
